  ...
  String _buildLocalAnswer({
    ...
    if (containsAny(['?????', '???????'])) {
      return compose(
        ...
        numbers:
            '?????? ??????? ${m(snap.actualTreasuryApproved)} | ???? ?????? ${m(snap.pendingNet)}',
        formula:
            '${m(snap.actualTreasuryApproved)} + ${m(snap.pendingNet)} = ${m(snap.availableLiquidityNow)}',
      );
    }
    ...
    if (containsAny(['????', '??? ?????'])) {
      return compose(
        ...
        numbers:
            '??? ${m(receivable)} | ????? ${m(payable)} | ???? ?????? ${m(snap.pendingNet)}',
        formula: '??? ????? ??????? = ?????? ??????? + (??? - ?????)',
      );
    }
    ...
    if (containsAny(['????', '???????'])) {
      return compose(
        ...
        formula:
            '???? ?????? = ${m(snap.pendingInflow)} - ${m(snap.pendingOutflow)} = ${m(snap.pendingNet)}',
      );
    }
    ...
    return {
      ...
      'liquidityFormulas': {
        ...
        'availableLiquidityNow':
            'actualTreasuryApproved + pendingInflow - pendingOutflow',
        'realCapitalApproved':
            'actualTreasuryApproved + (claimsReceivableOpen - claimsPayableOpen)',
      },
      ...
      'txnRules': {
        ...
        'fawry_cash': 'فوري نقدي: الدرج يزيد بصافي الربح.',
        'fawry_credit': 'فوري آجل: ينشئ مستحق لنا، والربح يُحتسب وقت الاعتماد.',
